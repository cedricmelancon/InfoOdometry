...
class DoubleHiddenTransitionModel(nn.Module):
    ...
    # Operates over (previous) state, (previous) poses, (previous) belief, (previous) nonterminals (mask), and
    ...
    def forward(self, prev_state, poses, prev_belief, observations):
        ...
        if self.args.belief_rnn == 'lstm':
            (prior_lstm_hiddens,
             posterior_lstm_hiddens) = [(torch.empty(0), torch.empty(0))] * T, [(torch.empty(0), torch.empty(0))] * T
            prior_lstm_hiddens[0] = (prev_belief.unsqueeze(0).repeat(2, 1, 1),
