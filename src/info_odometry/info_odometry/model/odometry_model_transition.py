...
class OdometryModelTransition(nn.Module):
    def __init__(self, args, belief_size, state_size, hidden_size, embedding_size, activation_function='relu'):
        ...
        self.rnn_embed_imu_hiddens = collections.deque(maxlen=args.clip_length + 1)
        self.fusion_lstm_hiddens = collections.deque(maxlen=args.clip_length + 1)

        self.out_features = collections.deque(maxlen=args.clip_length)
        self.pred_pose = collections.deque(maxlen=args.clip_length)
    ...
    def init_data(self, prev_belief):
        fusion_hiddens, fusion_features, out_features = [torch.empty(0)] * self.T, [torch.empty(0)] * self.T, [
            torch.empty(0)] * (self.T - 1)
        fusion_hiddens[0], fusion_features[0] = prev_belief, prev_belief
        if self.args.belief_rnn == 'lstm':
            fusion_lstm_hiddens = [torch.empty(0)] * self.T
            fusion_lstm_hiddens[0] = (prev_belief.unsqueeze(0).repeat(2, 1, 1),
    ...
    # s : -x--X--X--X--X--X-
    # @jit.script_method
    def forward(self, prev_belief, observations):
        """
        ...
        for t in range(T - 1):
            ...
            args_execution = {
                'rnn_embed_imu_hiddens': rnn_embed_imu_hiddens,
                'observations_imu': observations[1],
                'observations_visual': observations[0],
                'fusion_features': fusion_features,
                'fusion_lstm_hiddens': fusion_lstm_hiddens,
                't': t
            }
