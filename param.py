...
class Param:
    def __init__(self):
        ...
        self.parser.add_argument('--gumbel_tau_final', type=float, default=0.5,
        ...
        self.parser.add_argument('--stochastic_mode', type=str, default='none',
                                 help='v1, v2 or v3: details in notability-sen')
        ...
        self.parser.add_argument('--overshooting_kl_beta', type=float, default=0,
                                 help='latent overshooting kl weight for t > 1 (o to disable)')
        self.parser.add_argument('--overshooting_pose_scale', type=float, default=0,
                                 help='latent overshooting pose prediction weight for t > 1 (0 to disable)')
        self.parser.add_argument('--observation_beta', type=float, default=0,
        ...
        self.parser.add_argument('--direct_img', action='store_const', default=False, const=True,
                                 help='remove the image fc before the lstm')
        self.parser.add_argument('--fp16', action='store_const', default=False, const=True)
        self.parser.add_argument('--prefeat_type', type=str, default='out_conv6_1',
                                 help='prefeat layer name in flownet models')
        ...
        self.parser.add_argument('--eval_discard_num', type=int, default=0,
                                 help='the number of beginning frames to be discarded in eval')
        self.parser.add_argument('--sensors', type=str, default='img',
                                 help='img,imu,pose: whether use images, imu and last pose inputs => deprecated')
        ...
        self.parser.add_argument('--train_sequences', type=str, default='2012-01-25-12-14-25,2012-04-03-07-56-24', help='separated by , ')
        self.parser.add_argument('--train_sequences_gt', type=str, default='2012-01-25-12-14-25_part1_floor2,2012-04-03-07-56-24_part4_floor2',
                                 help='separated by , ')
        ...
        self.parser.add_argument('--euroc_ds_eval', type=str, default="both",
                                 choices=["downsample", "raw_freq", "both"])
        ...
        self.parser.add_argument('--t_euler_loss', action='store_const', default=True, const=True,
                                 help='otherwise use se3 loss')
        ...
        # ablation parameters
        self.parser.add_argument("--rec_target", type=str, default="flowfeat",
                                 choices=["flowfeat", "flow", "depth", "rgb"])
        self.parser.add_argument("--rec_weight", type=float, default=1.0)
        self.parser.add_argument('--rec_embedding_size', type=int, default=1024,
                                 help='embedding size for flownet feature reconstruction')
        self.parser.add_argument('--rec_activation_function', type=str, default='relu', choices=dir(F),
                                 help='model activation function')
        self.parser.add_argument('--rec_from_noise', action='store_const', default=False, const=True,
                                 help='use N(0,1) noise as posterior_states input for training')
        ...
        self.parser.add_argument("--vkitti2_eval_subscene", type=str, nargs='+',
                                 help="will only load --vkitti2_subscene and disable --vkitti2_clone_only")
        self.parser.add_argument("--flowfeat_size_dataset", type=str, default="mit",
        ...
        if self.args.eval:
            ...
            bk_args = ['gpu', 'on_the_fly', 'eval_discard_num', 'eval_batch_size', 'eval', 'eval_uncertainty',
                       'uncertainty_groups', 'eval_ckp', 'eval_failure', 'noise_std_factor', 'img_prefeat',
                       'failure_type', 'eval_euroc_interp', 'euroc_ds_eval', 'rec_flow_split', 'flowfeat_size_dataset',
                       'ckp_dir', 'eval_outname', 'exp_name', 'vkitti2_clone_only', 'vkitti2_eval_subscene']
            bk_vals = dict()
            for _arg in bk_args:
            ...
            if self.args.dataset == 'euroc':
                ...
                if self.args.train_sequences == "none":
                    raise NotImplementedError(
                        "--train_sequences has not default values for --dataset {}".format(self.args.dataset))
                if self.args.eval_sequences == "none":
                    raise NotImplementedError(
                        "--eval_sequences has not default values for --dataset {}".format(self.args.dataset))
        ...
        if self.args.dataset == 'euroc':
            for seq in self.args.train_sequences:
                if not os.path.isdir('{}{}'.format(self.args.base_dir, seq)):
                    raise ValueError(
                        'the specified train_sequence folder {}{} does not exist'.format(self.args.base_dir, seq))
