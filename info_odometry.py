...
class InfoOdometry:
    ...
    def forward(self,
        ...
        # [time, batch, 6]
        y_rel_poses = torch.stack(y_rel_pose_list, dim=0).type(torch.FloatTensor).to(device=self.args.device)
        ...
        (observation_loss,
         ...
         kl_loss) = self._forward_observation(beliefs,
                                              posterior_states,
                                              prior_states,
                                              prior_means,
                                              prior_std_devs,
        ...
        pose_trans_loss_y = F.mse_loss(pred_rel_poses[:, :, 1:2] * self.args.translation_weight,
                                       y_rel_poses[:, :, 1:2] * self.args.translation_weight,
                                       reduction='none').sum(dim=2).mean(dim=(0, 1))
        pose_rot_loss = F.mse_loss(pred_rel_poses[:, :, -1:] * self.args.rotation_weight,
                                   y_rel_poses[:, :, -1:] * self.args.rotation_weight,
                                   reduction='none').sum(dim=2).mean(dim=(0, 1))
