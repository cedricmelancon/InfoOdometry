...
class MitStataCenterClipDataset(torch.utils.data.Dataset):
    def __init__(self, args,
                 load_mit_rgb=False,
                 load_mit_depth=False,
                 clip_length=None,
                 overlap=None,
                 on_the_fly=None,
                 img_prefeat=None,
                 base_dir=None,
                 train_img_from_scratch=None):
        """
        ...
        self.base_dir = base_dir  # 'data/kitti/odometry/dataset TODO
        ...
        self.load_mit_rgb = load_mit_rgb
        self.load_mit_depth = load_mit_depth
        ...
        if self.train_img_from_scratch:
            ...
            self.render_size = [480, 640]  # (kitti_size // 64) * 64

        self.args = args
        assert self.args.clip_length
        self.clips = []
        self.loaded_imgs = dict()  # used when not using --on_the_fly and --img_prefeat none or self.load_mit_rgb
    ...
    def extend(self, mit_data):
        """
        -> cut the consecutive sub_sequences into clips with clip_length=5 by default
        -> save each clip as a datapoint
        -> mit_data.sub_seqs: A dict with keys {'imus', 'imgs', 'rel_poses', 'global_poses'}
        ...
        """
        print('getting clips of sequence {} ...'.format(mit_data.sequence))
        # self.img_transforms = mit_data.img_transforms
        clips = []
        for i_seq, sub_seq in enumerate(mit_data.sub_seqs):
            if len(sub_seq['imus']) < self.clip_length:
                # discard sub_sequences that are shorter than clip_length
                print('discard')
                continue
            if (not self.on_the_fly and self.load_mit_rgb) or (not self.on_the_fly and not self.use_img_prefeat):
                self.update_loaded_imgs(sub_seq)
            if not self.on_the_fly and self.load_mit_depth:
    ...
    def update_loaded_prefeats(self, clips):
        ...
        #        fpath = 'data/kitti/old_flownet_features/clip_length_{}/{}/{}.pt'.format(self.args.clip_length, fseq, flabel) # TODO
    ...
    def _get_img_prefeat(self, last_ts, curr_ts):
        ...
        if self.on_the_fly:
            return torch.load(
                'data/kitti/old_flownet_features/clip_length_{}/{}/{}.pt'.format(self.args.clip_length, fseq, flabel))
        else:
    ...
    def _get_img_pair(self, last_ts, curr_ts):
        ...
        if self.on_the_fly:
            if self.train_img_from_scratch:  # [3, 2, 376, 1241]
                raise NotImplementedError()
                r_img_features = get_mit_imgpair(last_ts, curr_ts, self.base_dir, img_transforms=self.img_transforms)
            else:  # use FlowNet2/C/S pretrained models
                r_img_features = get_mit_imgpair(last_ts, curr_ts, self.base_dir, render_size=self.render_size)
    ...
    def _get_depth_pair(self, last_ts, curr_ts):
        ...
        """
        if self.on_the_fly:
            r_depth_features = get_mit_depthpair(last_ts, curr_ts, self.base_dir, render_size=self.render_size)
...
class MitStataCenterDataset(torch.utils.data.Dataset):
    def __init__(self, sequence, sequence_gt, args, base_dir=None, t_euler_loss=None, debug=None,
                 train_img_from_scratch=None):
        """
    ...
    def read_data(self, base_dir, seq, seq_gt):
        """
        ...
        for _i, _imu in tqdm(enumerate(imus)):
            ...
            dt = TransformUtils.get_relative_pose(tq_i, tq_ip1)
...
def load_mit_clips(seqs=None, seqs_gt=None, batch_size=None, shuffle=None, overlap=None, args=None,
                   sample_size_ratio=None, load_mit_rgb=False, load_mit_depth=False):
    """
