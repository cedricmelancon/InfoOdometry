...
def bottle(f, x_tuple):
    ...
    y = f(
        *map(lambda x: x[0].view(x[1][0] * x[1][1], *x[1][2:]), zip(x_tuple, x_sizes)))  # f([time x batch, 3, 2, H, W])
    if type(y) == tuple:
        y_size = y[0].size()
            ...
            if use_pose_model:
                with torch.no_grad():
                    ...
                    if self.args.eval_uncertainty:
                        _plist = []
                        for k in range(100):
                            _plist.append(poses(posterior_means[t + 1] + posterior_std_devs[t + 1] * torch.randn_like(
                                posterior_means[t + 1])))
...
class DoubleHiddenTransitionModel(nn.Module):
    ...
    def forward(self, prev_state, poses, prev_belief, observations):
        ...
        use_pose_model = True if type(poses) == PoseModel else False  # type(poses) == torch.Tensor # [time, batch, 6]
        T = self.args.clip_length + 1  # number of time steps # self.args.clip_length = poses.size(0)
        prior_beliefs, posterior_beliefs, prior_states, prior_means, prior_std_devs, posterior_states, posterior_means, posterior_std_devs = [
                                                                                                                                                 torch.empty(
                                                                                                                                                     0)] * T, [
                                                                                                                                                 torch.empty(
                                                                                                                                                     0)] * T, [
                                                                                                                                                 ...
                                                                                                                                                 torch.empty(
                                                                                                                                                     0)] * T
        prior_beliefs[0], posterior_beliefs[0], prior_states[0], posterior_states[
            0] = prev_belief, prev_belief, prev_state, prev_state
        if self.args.belief_rnn == 'lstm':
        ...
        for t in range(T - 1):
            ...
            if self.imu_only:
                hidden, rnn_embed_imu_hiddens[t + 1] = self.rnn_embed_imu(observations[t_ + 1],
                                                                          rnn_embed_imu_hiddens[t])
                hidden = self.act_fn(self.fc_embed_state_posterior(
                    torch.cat([hidden[:, -1, :], posterior_states[t], prior_states[t]], dim=1)))
            else:  # image features
                hidden = self.act_fn(self.fc_embed_state_posterior(
                    torch.cat([observations[t_ + 1], posterior_states[t], prior_states[t]], dim=1)))
...
class DoubleStochasticVITransitionModel(nn.Module):
    def __init__(self, args, belief_size, state_size, hidden_size, embedding_size, activation_function='relu',
        ...
        self.min_std_dev = min_std_dev  # 6 * args.pose_tiles + embedding_size
        self.stochastic_mode = args.stochastic_mode
        if args.belief_rnn == 'lstm': raise NotImplementedError(
            'do not support belief_rnn lstm for double-vinet-stochastic')
    ...
    def forward(self, prev_state, poses, prev_belief, observations):
        ...
        for t in range(T - 1):
            ...
            if self.stochastic_mode == 'v1':
                hidden = self.act_fn(
                    self.fc_embed_state_posterior(torch.cat([observations_visual[t_ + 1], imu_feat], dim=1)))
                posterior_beliefs[t + 1] = self.rnn_posterior(hidden,
                                                              torch.cat([posterior_states[t], prior_states[t]], dim=1))
                posterior_means[t + 1], _posterior_std_dev = torch.chunk(posterior_beliefs[t + 1], 2, dim=1)
                posterior_std_devs[t + 1] = F.softplus(_posterior_std_dev) + self.min_std_dev
                ...
                hidden = self.act_fn(self.fc_embed_state_posterior(
                    torch.cat([observations_visual[t_ + 1], imu_feat, prior_states[t]], dim=1)))
                posterior_beliefs[t + 1] = self.rnn_posterior(hidden,
                                                              torch.cat([posterior_states[t], posterior_states[t]],
                                                                        dim=1))
                posterior_means[t + 1], _posterior_std_dev = torch.chunk(posterior_beliefs[t + 1], 2, dim=1)
...
class MultiHiddenVITransitionModel(nn.Module):
    ...
    def forward(self, prev_state, poses, prev_belief, observations):
        ...
        use_pose_model = True if type(poses) == PoseModel else False  # type(poses) == torch.Tensor # [time, batch, 6]
        T = self.args.clip_length + 1  # number of time steps # self.args.clip_length = poses.size(0)
        prior_beliefs, posterior_beliefs_visual, posterior_beliefs_imu, prior_states, prior_means, prior_std_devs, posterior_states, posterior_means, posterior_std_devs = [
                                                                                                                                                                               torch.empty(
                                                                                                                                                                                   0)] * T, [
                                                                                                                                                                               torch.empty(
                                                                                                                                                                                   0)] * T, [
        ...
        if self.args.belief_rnn == 'lstm':
            prior_lstm_hiddens, posterior_lstm_hiddens_visual, posterior_lstm_hiddens_imu = [(torch.empty(0),
                                                                                              torch.empty(0))] * T, [(
                                                                                                                     torch.empty(
                                                                                                                         0),
                                                                                                                     torch.empty(
                                                                                                ...
                                                                                                (torch.empty(0),
                                                                                                 torch.empty(0))] * T
            prior_lstm_hiddens[0] = (prev_belief.unsqueeze(0).repeat(2, 1, 1), prev_belief.unsqueeze(0).repeat(2, 1, 1))
            posterior_lstm_hiddens_visual[0] = (
            prev_belief.unsqueeze(0).repeat(2, 1, 1), prev_belief.unsqueeze(0).repeat(2, 1, 1))
            posterior_lstm_hiddens_imu[0] = (
            prev_belief.unsqueeze(0).repeat(2, 1, 1), prev_belief.unsqueeze(0).repeat(2, 1, 1))
        ...
        for t in range(T - 1):
            ...
            if self.args.belief_rnn == 'gru':
                ...
                hidden = hidden.unsqueeze(1)
                hidden, posterior_lstm_hiddens_visual[t + 1] = self.rnn_visual_posterior(hidden,
...
class PoseModel(nn.Module):
    def __init__(self, args, belief_size, state_size, hidden_size, embedding_size, activation_function='relu'):
        ...
        self.fc3_lin = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, state):
        ...
        trans_x = self.fc4_trans_x(F.relu(self.dropout(self.fc3_lin(hidden_lin), 0.5)))
        trans_y = self.fc3_trans_y(hidden_ang)
        zero_trans = torch.zeros([trans_x.shape[0], 1]).to('cuda:1')
        rot = self.fc4_rot(self.act_fn(self.dropout(self.fc3_rot(hidden_ang), 0.5)))
